...
export async function invokeSg50StatusQuery() {
  ...
  const capabilities = await getSg50Capabilities(deviceId);
  const service = capabilities.statusQueryService;
  ...
  // Never accept a service ID from the browser. Only invoke the safe,
  // automatically detected zero-input status/report query from the device TSL.
  const payload = await milesightRequest(
    `/device/openapi/v1/devices/${encodeURIComponent(
      deviceId
    )}/services/call`,
